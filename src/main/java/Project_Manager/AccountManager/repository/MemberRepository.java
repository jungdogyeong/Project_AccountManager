...
public class MemberRepository {
     ...
     * 로그인 ID로 회원 조회
     */
    public Optional<MemberDomain> findByLoginId(String loginId) {
        String sql = "SELECT id, login_id, password FROM MEMBER WHERE login_id = ?";
        try {
            MemberDomain member = jdbcTemplate.queryForObject(sql, memberRowMapper(), loginId);
    ...
    private RowMapper<MemberDomain> memberRowMapper() {
        return (rs, rowNum) -> {
